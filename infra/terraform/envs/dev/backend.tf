# État distant de l'environnement dev.
#
...
#
#   terraform init -backend-config="kms_key_id=<uuid de la clé dev>"

terraform {
  backend "s3" {
    bucket         = "spa-booking-dev-tfstate"
    key            = "envs/dev/terraform.tfstate"
    region         = "eu-west-3"
    dynamodb_table = "spa-dev-tflock"
    encrypt        = true
  }
}
