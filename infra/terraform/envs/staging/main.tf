...
module "notifications" {
  ...
  # SMS et e-mail —, que le rôle de tâche de l'API reçoit plus bas. Le droit
  # d'envoyer est propre à l'environnement, le plafond ne l'est pas.
  #
  # Et le sender ID avec elles (#918) : l'expéditeur est un attribut de chaque
  # publication, pas un réglage de compte. C'est ce qui permet d'éprouver le
  # canal SMS en recette — le seul endroit où l'on peut envoyer pour de vrai sans
  # toucher une cliente — sans que la recette détienne les préférences du compte.
...
module "ecs_service" {
  ...
  services = {
    api = {
      ...
      # 503 et inscrivent une ligne `FAILED` par message — et la recette
      # n'éprouverait jamais la chaîne qu'elle est là pour éprouver.
      #
      ...
      # SMS du compte (#66) ; la troisième borne `ses:SendEmail` à l'identité de
      # domaine de cet environnement et à son jeu de configuration, sans quoi
      # l'appel de #799 reviendrait en `AccessDenied` (#918).
      #
