...
| `tenant-scope.isolation-spec` en échec (« Authentication failed … credentials … not valid ») alors que les autres suites d'intégration passent | les identifiants du volume `postgres-data` local précèdent le `docker-compose.yml` courant — `POSTGRES_PASSWORD` n'agit qu'à l'initialisation du volume | aligner le `.env` local sur le compose. **Ne jamais recréer le volume pendant qu'une vague tourne** — jusqu'à trois agents partagent ce conteneur, et `docker compose down -v` couperait la base sous eux. Hors vague seulement : `docker compose down -v && docker compose up -d` puis `npm run db:migrate:deploy` |
