...
| « Authentication failed … credentials … not valid » alors que le conteneur **tourne et est sain** | un autre serveur PostgreSQL tient le port visé : deux processus peuvent lier `0.0.0.0:5432` sans erreur, et la connexion part vers celui qui n'a pas les identifiants du compose (#272) | `netstat -ano \| grep :5433` (le port publié par le compose) — s'il y a deux PID à l'écoute, c'est cela. Le dépôt publie le sien sur **5433** : vérifier que `DATABASE_URL` le vise, dans `.env.local` comme dans le shell, et non le 5432 qu'un PostgreSQL natif peut tenir |
