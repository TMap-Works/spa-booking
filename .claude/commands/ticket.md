...
| « Authentication failed … credentials … not valid » alors que le conteneur **tourne et est sain** | un autre serveur PostgreSQL tient le port visé : deux processus peuvent lier `0.0.0.0:5432` sans erreur, et la connexion part vers celui qui n'a pas les identifiants du compose (#272) | `netstat -ano \| grep :5433` (le port publié par le compose) — s'il y a deux PID à l'écoute, c'est cela. Le dépôt publie le sien sur **5433** : vérifier que `DATABASE_URL` le vise et non le 5432 qu'un PostgreSQL natif peut tenir. Le seul fichier d'environnement versionné est `.env.example`, et un worktree n'hérite d'aucun `.env.local` — c'est donc la variable du shell qui fait foi, et c'est là qu'il faut la corriger, pas dans un fichier à créer |
| `tenant-scope.isolation-spec` en échec (« Authentication failed … ») alors que les autres suites d'intégration passent **et** qu'un seul PID écoute | les identifiants du volume `postgres-data` local précèdent le `docker-compose.yml` courant — `POSTGRES_PASSWORD` n'agit qu'à l'initialisation du volume | `ALTER USER spa WITH PASSWORD 'spa'` dans le conteneur suffit et ne détruit rien. **Ne jamais recréer le volume pendant qu'une vague tourne** — jusqu'à trois agents partagent ce conteneur, et `docker compose down -v` couperait la base sous eux |
