...
describe('Écritures de rendez-vous au comptoir', () => {
  ...
  describe('POST /api/v1/appointments/:id/status — faire avancer', () => {
     ...
     *
     * Le recul inclut le **tampon de préparation** : la colonne porte l'intervalle
     * occupé, et le soin ne commence qu'un tampon plus tard (`billed-interval.ts`)
     * — c'est cette heure-là que la règle compare à l'horloge. Reculer du seul
     * délai voulu ferait basculer ces cas en 422 le jour où le décor se donnerait
     * un tampon plus long que lui, pour une raison qui n'aurait rien à voir.
     */
    function commencé(): string {
      const startsAt = new Date(Date.now() - (30 + BUFFER_BEFORE_MINUTES) * MINUTE_MS);
