 ...
 *
 * Les cinq routes de `/api/v1/staff-time-off` y sont exercées pour ce que
...
describe('Plages bloquées et congés du personnel', () => {
  ...
  describe('le module est servi par AppModule', () => {
    ...
    it('sert les quatre autres chemins du contrôleur, pas seulement la liste', async () => {
  ...
  describe('GET /staff-time-off', () => {
    ...
    it('écarte les absences seulement adjacentes à la fenêtre', async () => {
      // Convention `[from, to[` du module : une absence qui s'achève **à**
      // `from`, ou qui commence **à** `to`, ne recoupe pas la fenêtre. Sans ces
      // deux bornes, un `>=` ou un `<=` glissé dans le prédicat de recoupement
      // passerait inaperçu — et le planning montrerait des absences voisines du
      // mois affiché.
      await seedTimeOff({ startsAt: '2026-07-25T00:00:00Z', endsAt: WINDOW.from });
      await seedTimeOff({ startsAt: WINDOW.to, endsAt: '2026-09-05T00:00:00Z' });
  ...
  describe('GET /staff-time-off/:id', () => {
    ...
    it('refuse le rang CLIENT', async () => {
      // Le seuil de cette route se vérifie comme celui de la liste : sans ce
      // cas, ramener `@AuthAtLeast('STAFF')` à `CLIENT` sur la lecture par
      // identifiant laisserait la suite entière au vert, et le motif interne
      // d'une absence deviendrait lisible depuis le parcours client.
      const created = await seedTimeOff({ reason: 'Formation' });
