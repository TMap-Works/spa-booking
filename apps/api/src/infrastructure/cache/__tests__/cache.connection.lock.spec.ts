...
class FakeRedis {
  ...
  public hang = false;
  /**
   * Les commandes ne se règlent que sur ordre — exerce le Redis **lent**, celui
   * qui répond après que l'appelant a cessé de l'attendre.
   */
  public defer = false;

  private pending: ((reply: unknown) => void)[] = [];

  /** Règle toutes les commandes différées, dans l'ordre où elles sont parties. */
  public settle(reply: unknown): void {
  ...
  private record<T>(command: SentCommand, reply: () => T): Promise<T> {
    ...
    if (this.defer) {
      return new Promise<T>((resolve) => {
        this.pending.push((forced) => resolve(forced as T));
      });
    }
...
describe('CacheConnection — prise de verrou', () => {
  ...
  it('relâche le verrou qu’il a cessé d’attendre, quand la commande finit par se poser', async () => {
    // Abandonner n'annule pas : la commande est déjà partie sur la socket. Sans
    // cette libération, un Redis **lent** laisserait la clé tenue par un jeton
    // que plus personne ne détient — et refuserait le créneau pendant tout le
    // TTL, à des appelants qui n'ont aucun concurrent.
    jest.useFakeTimers();
    mockClient.defer = true;
    ...
    try {
      ...
      // Le `SET` retardataire arrive : la clé vient d'être posée pour dix
      // secondes, et personne n'en tient le jeton.
      mockClient.defer = false;
      mockClient.settle('OK');
      await jest.advanceTimersByTimeAsync(0);

      const [name, , sentToken] = mockClient.sent[0] as [string, string, string, ...unknown[]];
  ...
  it('ne relâche rien quand la commande abandonnée a fini par échouer', async () => {
    // Rien n'a été posé : un `DEL` conditionnel de plus ne ferait qu'un
    // aller-retour inutile sur un cache déjà en peine.
    jest.useFakeTimers();
    mockClient.hang = true;
