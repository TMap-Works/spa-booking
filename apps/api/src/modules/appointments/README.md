...
La **borne** et le **nom de ce détail** viennent du contrat partagé —
`hasAppointmentStarted` et `APPOINTMENT_NOT_STARTED_DETAIL` de `@spa/shared`,
posés par #1210. Ce sont les mêmes que lisent le tiroir du comptoir et « Mon
planning » pour n'ouvrir les deux constats qu'à partir de l'heure : écrites des
deux côtés, une inégalité finit par diverger d'un `<` et une clé par être
renommée d'un seul côté — un écran offrirait alors un geste que cette règle
refuse, ou n'en reconnaîtrait plus le refus.
